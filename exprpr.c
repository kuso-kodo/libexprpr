...
int pr_error_code(void) {
    return __prprprpr_error;
}

const char *pr_error_text(void) {
    if(__prprprpr_error == 0)
        return "No error.";
    if(__prprprpr_error_text == NULL)
...
void pr_free_arguments(pr_expr *expr) {
    if (expr == NULL)
        return;
    switch (expr->type) {
        case PR_FUNCTION3: pr_free(expr->arguments[2]);     // fall-through
        case PR_FUNCTION2: pr_free(expr->arguments[1]);     // fall-through
        case PR_FUNCTION1: pr_free(expr->arguments[0]);
    }
    return;
}
...
static const pr_var *get_builtin_token(const char *name, int len) {
    ...
    for (var = __built_in_var_table, i = sizeof(__built_in_var_table) / sizeof(pr_var) - 1; i > 0; var++, i--) {
...
// "Magic method"
static double __add__(double a, double b) {
    return a + b;
}

static double __sub__(double a, double b) {
    return a - b;
}

static double __mul__(double a, double b) {
    return a * b;
}
...
static double __neg__(double a) {
    return -a;
}

static double __comma__(double a, double b) {
    return b;
}
    ...
    while(s->type == TOK_NUL) {
        ...
        if(isdigit(s->next[0]) || s->next[0] == '.') {
            ...
            const char *start;
                ...
                if(temp == NULL)
                    temp = get_builtin_token(start, s->next - start);
            ...
            switch (s->next[0]) {
                    ...
                    case '/': s->type = TOK_IFX; s->func = __div__; break;
                    case '^': s->type = TOK_IFX; s->func = pow; break;
                    case '%': s->type = TOK_IFX; s->func = fmod; break;
                    case '(': s->type = TOK_OPN; break;
                    case ')': s->type = TOK_CLO; break;
                    case ',': s->type = TOK_SEP; break;
                    default: s->type = TOK_ERR; SET_ERR_TEXT(ERR_UNKNOWN_SYMBOL); break;
                }
            s->next++;
        }
    }
...
static pr_expr *term(state *s) {
    ...
    while(s->type == TOK_IFX && (s->func == __mul__ || s->func == __div__ || s->func == fmod)) {
        pr_func2 temp = s->func;
        next_token(s);
...
pr_expr *pr_parse(const char *c_expr, const pr_var *var_table, int var_count) {
    ...
    if (s.type != TOK_END) {
        ...
        if (__prprprpr_error  == 0)
            __prprprpr_error  = -1; // Error at position 0.
        return NULL;
    }else {
        __prprprpr_error = 0;
        return root;
...
double pr_eval(const pr_expr *expr) {
    ...
    switch(expr->type) {
        case PR_CONSTANT: return expr->value;
        case PR_VARIABLE: return *expr->refer;

        case PR_FUNCTION0: case PR_FUNCTION1: case PR_FUNCTION2: case PR_FUNCTION3:
            switch(ARGC(expr->type)) {
                case 0: return FUNC(void)();
                case 1: return FUNC(double)(pr_eval(expr->arguments[0]));
                case 2: return FUNC(double, double)(pr_eval(expr->arguments[0]), pr_eval(expr->arguments[1]));
                case 3: return FUNC(double, double, double)(pr_eval(expr->arguments[0]), pr_eval(expr->arguments[1]),pr_eval(expr->arguments[2]));
                default: return NAN;
            }
        default:
            return NAN;
